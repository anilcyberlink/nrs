...
class FrontpageController extends Controller
{
  ...
  public function become_member(Request $request)
  {
    ...
    if ($request->isMethod('post')) {
      // dd($request->all());
      $activeevent = EventModel::where('id', $request->event)->first();
      if($activeevent->is_open == 2){
        
        return redirect('/')->with([
          'type' => 'info',
          'message' => 'Registration opens soon. Stay connected for updates!'
        ]);
      }
      if($activeevent->is_open == 0){
        
        return redirect('/')->with([
          'type' => 'info',
          'message' => 'Registration closed. Stay connected for updates!'
        ]);
      }
      $request->validate([
        'first_name' => 'required',
