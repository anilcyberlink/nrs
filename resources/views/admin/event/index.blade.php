									...
									@if(count($data) > 0)
										@foreach($data as $row)
											<tr class="id{{$row->id}}">
												...
												<td><a href="{{ url('admin/event/' . $row->id) }}">{{ ucfirst($row->name) }}</a>
													<br>
													<span >
														@if ($row->is_open == 2)
															(Upcoming)
														@elseif ($row->is_open == 1)
															(Ongoing)
														@else
															(Completed)
														@endif
												...
												<td class="">
													<form action="{{route('event.isdefault', $row->id)}}" method="POST">
														@csrf
														@if(($row->status) == 0)
															<button class="btn btn-danger btn btn-sm" name="status" value="0"
																type="submit"><i class="fa fa-times"></i>
															</button>
														@else
															<button class="btn btn-success btn btn-sm" name="status" value="1"
												...
												<td class="text-center">
													...
													@if(!is_empty_event($row->id)) |
														<a href="{{ url('admin/event/' . $row->id . '/destroy') }}"
															onclick="return confirm('Confirm Delete?')" class="btn-delete">
															Delete
														</a>
													@endif
												</td>
